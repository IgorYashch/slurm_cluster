...
X_train = pd.read_pickle('data/version_2/X_train.pickle')
X_test = pd.read_pickle('data/version_2/X_test.pickle')
y_train = pd.read_pickle('data/version_2/y_train.pickle')
y_test = pd.read_pickle('data/version_2/y_test.pickle')
...
def count_results(models, X_train, y_train, X_test, y_test, target_scaler):
    ...
    return results, results_unscaled

results, results_unscaled = count_results(models, X_train, y_train, X_test, y_test, target_scaler)

exp_num = 10

results.to_csv(f'exp_{exp_num}/results_scaled.csv')
results_unscaled.to_csv(f'exp_{exp_num}/results_unscaled.csv')

X_train.to_csv(f'exp_{exp_num}/X_train.csv.gz', compression='gzip')
y_train.to_csv(f'exp_{exp_num}/y_train.csv.gz', compression='gzip')
X_test.to_csv(f'exp_{exp_num}/X_test.csv.gz', compression='gzip')
y_test.to_csv(f'exp_{exp_num}/y_test.csv.gz', compression='gzip')

    
with open(f'exp_{exp_num}/y_scaler.pickle', 'wb') as f:
    pickle.dump(target_scaler, f)
    
for model_name in models:
    new_name = model_name.replace(' ', '_')
    with open(f'exp_{exp_num}/{new_name}.pickle', 'wb') as f:
...
results_unscaled.T[['MSE train','MSE test', 'MAE train', 'MAE test', 'MAPE train',
       'MAPE test', 'time']]
results_unscaled.T[['MSE train','MSE test', 'MAE train', 'MAE test', 'MAPE train', 'MAPE test', 'time']].to_csv('results_classical_ml/results_unscaled.csv')

results.T[['MSE train','MSE test', 'MAE train', 'MAE test', 'MAPE train', 'MAPE test', 'time']].to_csv('results_classical_ml/results.csv')

results_unscaled = pd.read_csv('results_classical_ml/results_unscaled.csv', index_col=0)[['MSE test', 'MAE test', 'MAPE test', 'time']]


results_scaled = pd.read_csv('results_classical_ml/results.csv', index_col=0)[['MSE test', 'MAE test', 'MAPE test', 'time']]
...
with open(f'exp_10/LGBM.pickle', 'rb') as f:
    model = pickle.load(f)

with open(f'exp_10/Random_Forest_4.pickle', 'rb') as f:
    model = pickle.load(f)
