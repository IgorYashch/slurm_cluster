...
RUN \
    ...
    yum -y install --setopt=tsflags=nodocs \
        ...
        munge munge-devel \
        readline readline-devel curl-devel \
        hdf5 hdf5-devel pam-devel hwloc hwloc-devel \
        perl perl-ExtUtils-MakeMaker python3

# source of slurm
ENV SLURM_TAR_BZ2_SOURCE=https://download.schedmd.com/slurm/slurm-21.08.8.tar.bz2
...
COPY ./docker/make_slurm_rpms ./docker/make_plugin ./docker/utils/cmd_setup ./docker/utils/cmd_start ./docker/utils/cmd_stop /usr/local/sbin/
ENTRYPOINT ["/usr/local/sbin/cmd_start"]
CMD ["make_slurm_rpms", "make_plugin"]
